...
def demo_os_detection():
    ...
    if os_info['os'] == 'Linux':
        ...
        if distro:
            print(f"   Distribuição: {distro.get('name', 'Desconhecida')} {distro.get('version', '')}")
...
def demo_gpu_detection():
    ...
    for i, gpu in enumerate(gpus, 1):
        ...
        if gpu.cuda_compatible:
            ...
            print(f"      Versões CUDA Recomendadas: {', '.join(gpu.recommended_cuda_versions)}")
...
def demo_comprehensive_analysis():
    ...
    print(f"   Sistema: {os_info['os']} {os_info['version']} ({os_info['architecture']})")
    ...
    print(f"\n✅ Compatibilidade:")
    print(f"   Python 3.13: {'✅ Compatível' if compatibility['python_313'] else '❌ Incompatível'}")
    print(f"   CUDA: {'✅ Suportado' if compatibility['cuda_capable'] else '❌ Não suportado'}")
    print(f"   Admin: {'✅ Disponível' if compatibility['has_admin'] else '❌ Necessário'}")
...
Exemplos de uso:
  ...
  python demo.py --analysis         # Apenas análise completa
    ...
    parser.add_argument("--all", action="store_true",
                       help="Executar todas as demonstrações")
    parser.add_argument("--quick", action="store_true",
                       help="Demonstrações rápidas essenciais")
    parser.add_argument("--analysis", action="store_true",
                       help="Apenas análise completa do sistema")
    parser.add_argument("--no-banner", action="store_true",
                       help="Não exibir banner")
