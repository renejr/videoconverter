...
class VideoConverterInstaller:
    ...
    def analyze_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis["components"]["python_installations"] = self.python_manager.get_current_python()
            ...
            if self.python_manager.needs_python_install():
                ...
                analysis["recommendations"].append("Sistema pode não suportar Python 3.13")
            
            # 2. Verificar GPU
            progress_bar.update(2, description="Detectando GPU")
            gpus = self.gpu_detector.detect_gpus()
            if gpus:
                analysis["components"]["gpu_info"] = self.gpu_detector.get_best_gpu(gpus)
                analysis["system"]["compatibility"]["cuda_support"] = analysis["components"]["gpu_info"].is_cuda_capable
            
            # 3. Verificar CUDA
            progress_bar.update(3, description="Verificando CUDA")
            analysis["components"]["cuda_installations"] = self.cuda_installer.detect_installed_cuda()
            
            # 4. Verificar FFmpeg
            progress_bar.update(4, description="Verificando FFmpeg")
            analysis["components"]["ffmpeg_installations"] = self.ffmpeg_manager.detect_installed_ffmpeg()
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        python_313_found = (
            python_info and 
            python_info.version.startswith("3.13")
        )
        ...
        if analysis["system"]["compatibility"]["cuda_support"]:
            cuda_installed = any(
                inst.is_complete for inst in analysis["components"]["cuda_installations"]
            )
            ...
            if not cuda_installed:
                ...
                if gpu_info and gpu_info.recommended_cuda_versions:
                    installed_versions = [inst.version for inst in analysis["components"]["cuda_installations"]]
                    compatible_installed = any(
                        any(rec_ver.startswith(inst_ver[:4]) for rec_ver in gpu_info.recommended_cuda_versions)
                        for inst_ver in installed_versions
                    )
                    
                    if not compatible_installed:
                        recommendations.append("Atualizar CUDA para versão compatível com GPU")
    ...
    def create_installation_plan(self, 
                                ...
                                custom_config: Dict[str, Any] = None) -> InstallationPlan:
        """
        ...
        self.logger.info(f"  CUDA: {'✅' if plan.install_cuda else '❌'} {plan.cuda_version or ''}")
    ...
    def _interactive_planning(self, analysis: Dict[str, Any]) -> InstallationPlan:
        ...
        if analysis["system"]["compatibility"]["cuda_support"]:
            ...
            if not cuda_installed:
                response = input("\n🚀 Instalar CUDA para aceleração GPU? [S/n]: ").strip().lower()
        ...
        # Modo offline
        response = input("\n💾 Habilitar modo offline (cache downloads)? [s/N]: ").strip().lower()
    ...
    def execute_installation(self, plan: InstallationPlan = None) -> InstallationResult:
        ...
        try:
            ...
            if plan.install_python:
                component_count += 1
                main_progress.update(component_count, description="Instalando Python 3.13")
            ...
            if plan.install_cuda:
                component_count += 1
                main_progress.update(component_count, description=f"Instalando CUDA {plan.cuda_version}")
                ...
                if self._install_cuda(plan):
                    ...
                    result.errors.append(f"Falha na instalação do CUDA {plan.cuda_version}")
                    self.logger.error(f"❌ Falha na instalação do CUDA {plan.cuda_version}")
            ...
            if result.components_failed:
                main_progress.cancel(f"Instalação concluída com {len(result.components_failed)} falha(s)")
            else:
            ...
            # Log final
            if result.success:
                self.logger.success(f"🎉 Instalação concluída com sucesso em {result.installation_time:.1f}s")
            else:
                self.logger.error(f"⚠️  Instalação concluída com falhas em {result.installation_time:.1f}s")
    ...
    def _install_python(self, plan: InstallationPlan) -> bool:
        ...
        try:
            ...
            # Instalar Python
            success = self.python_manager.install_python(installer_path, plan.python_version)
    ...
    def _install_cuda(self, plan: InstallationPlan) -> bool:
        ...
        try:
            if not plan.cuda_version:
                ...
                if not plan.cuda_version:
                    self.logger.error("Não foi possível determinar versão CUDA compatível")
                    return False
            ...
            # Instalar CUDA
            success = self.cuda_installer.install_cuda(installer_path, plan.cuda_version)
    ...
    def _advanced_validation(self, plan: InstallationPlan) -> Dict[str, bool]:
        ...
        if plan.install_python:
            results["python_version"] = self.python_manager.verify_installation(plan.python_version)
        
        if plan.install_cuda:
            results["cuda_functionality"] = self.cuda_installer.verify_installation(plan.cuda_version)
    ...
    def _benchmark_validation(self, plan: InstallationPlan) -> Dict[str, bool]:
        ...
        if plan.install_python:
            try:
                ...
                result = subprocess.run([
                    "python", "-c", 
                    "import time; sum(i*i for i in range(100000))"
                ], capture_output=True, timeout=30)
                
                execution_time = time.time() - start_time
                results["python_performance"] = result.returncode == 0 and execution_time < 10
        ...
        if plan.install_ffmpeg:
            try:
                ...
                result = subprocess.run([
                    ...
                    "-t", "1", "-y", test_file.replace(".txt", ".mp4")
                ], capture_output=True, timeout=30)
    ...
    def _generate_final_recommendations(self, result: InstallationResult) -> List[str]:
        ...
        if result.components_failed:
            ...
            recommendations.append("Tentar instalação manual dos componentes que falharam")
        
        if result.success:
            recommendations.append("Reiniciar terminal/sistema para aplicar mudanças de PATH")
            recommendations.append("Testar VideoConverter com um arquivo de exemplo")
            
            # Recomendações específicas por validação
            validation_failed = [k for k, v in result.validation_results.items() if not v]
            if validation_failed:
                recommendations.append(f"Verificar componentes com falha na validação: {', '.join(validation_failed)}")
    ...
    def generate_report(self, result: InstallationResult) -> str:
        ...
        report.append(f"📊 STATUS GERAL: {status_icon} {'SUCESSO' if result.success else 'FALHA'}")
...
Exemplos de uso:
  ...
  python main_installer.py --validate-only    # Apenas validar
    ...
    parser.add_argument("--auto", action="store_true",
                       help="Modo automático (sem interação)")
    parser.add_argument("--config", type=str,
                       help="Arquivo de configuração JSON")
    parser.add_argument("--cache-dir", type=str,
                       help="Diretório de cache personalizado")
    parser.add_argument("--offline", action="store_true",
                       help="Modo offline (usar cache)")
