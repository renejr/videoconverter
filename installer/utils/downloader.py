...
class DownloadCache:
    ...
    def is_cached(self, url: str, filename: str = None, verify_integrity: bool = True) -> bool:
        """
    ...
    def add_to_cache(self, url: str, filepath: str, download_info: DownloadInfo = None):
        ...
        """
        cache_path = self.get_cache_path(url, download_info.filename if download_info else None)
        ...
        try:
            ...
            if download_info:
                ...
                if download_info.sha256:
                    self.metadata[cache_key]["sha256"] = self._calculate_hash(cache_path, "sha256")
    ...
    def get_from_cache(self, url: str, destination: str, filename: str = None) -> bool:
        ...
        try:
            dest_path = os.path.join(destination, filename or os.path.basename(cache_path))
        ...
        if total_size > max_size_bytes:
            ...
            for cache_key, cache_path, _ in remaining_files:
                ...
                try:
                    ...
                    del self.metadata[cache_key]
                    total_size -= file_size
                    self.logger.info(f"Arquivo removido por limite de tamanho: {cache_key}")
                except Exception as e:
    ...
    def _verify_integrity(self, filepath: str, download_info: DownloadInfo) -> bool:
        ...
        if download_info.md5:
            ...
            if file_md5.hexdigest().lower() != download_info.md5.lower():
                self.logger.error(f"MD5 não confere: esperado {download_info.md5}, obtido {file_md5.hexdigest()}")
                return False
        ...
        if download_info.sha256:
            ...
            if file_sha256.hexdigest().lower() != download_info.sha256.lower():
                self.logger.error(f"SHA256 não confere: esperado {download_info.sha256}, obtido {file_sha256.hexdigest()}")
                return False
    ...
    def _download_with_resume(
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=self.timeout, context=self.ssl_context) as response:
                # Obter tamanho total
                content_length = response.headers.get('Content-Length')
                total_size = int(content_length) if content_length else download_info.size
    ...
    def download(
        ...
        if self.use_cache and not force_download:
            if self.cache.is_cached(download_info.url, filename):
                ...
                if self.cache.get_from_cache(download_info.url, destination_dir, filename):
                    return DownloadResult(
                        success=True,
        ...
        for attempt, url in enumerate(urls_to_try):
            ...
            for retry in range(self.max_retries):
                ...
                if result.success:
                    # Adicionar ao cache
                    if self.use_cache:
                        self.cache.add_to_cache(download_info.url, destination_path, download_info)
                    
                    self.logger.success(f"Download concluído: {filename}")
                    return result
                
                last_error = result.error
                self.logger.warning(f"Falha no download (tentativa {retry + 1}): {result.error}")
        
        # Todas as tentativas falharam
        return DownloadResult(
            success=False,
            error=f"Falha após todas as tentativas: {last_error}"
        )
        ...
        with ThreadPoolExecutor(max_workers=max_concurrent) as executor:
            ...
            for future in as_completed(future_to_info):
                ...
                try:
                    ...
                    if result.success:
                        self.logger.success(f"Download concluído: {download_info.filename}")
                    else:
                        self.logger.error(f"Falha no download: {download_info.filename} - {result.error}")
                        
                except Exception as e:
                    self.logger.error(f"Erro no download: {download_info.filename} - {e}")
