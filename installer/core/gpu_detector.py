...
class GPUDetector:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.logger.debug(
                "nvidia-ml-py3 não disponível, usando métodos alternativos"
            )
    ...
    def _run_command(self, command: List[str]) -> Tuple[bool, str, str]:
        ...
        try:
            result = subprocess.run(
                ...
                timeout=30,
                creationflags=(
                    subprocess.CREATE_NO_WINDOW if self.os_type == "windows" else 0
                ),
            )
    ...
    def _detect_via_nvml(self) -> List[GPUInfo]:
        ...
        try:
            ...
            for i in range(device_count):
                ...
                # Driver e CUDA
                driver_version = self.pynvml.nvmlSystemGetDriverVersion().decode(
                    "utf-8"
                )
    ...
    def _detect_via_nvidia_smi(self) -> List[GPUInfo]:
        ...
        try:
            ...
            for i, gpu_elem in enumerate(root.findall(".//gpu")):
                ...
                driver_version = (
                    driver_elem.text if driver_elem is not None else "Unknown"
                )
    ...
    def detect_gpus(self) -> List[GPUInfo]:
        ...
        for method_name, method_func in methods:
            try:
                ...
                if detected_gpus:
                    self.logger.info(
                        f"Detectadas {len(detected_gpus)} GPU(s) via {method_name}"
                    )
        ...
        for gpu in gpus:
            gpu.recommended_cuda_versions = self._get_recommended_cuda_versions(
    ...
    def _get_recommended_cuda_versions(
        self, compute_capability: Tuple[int, int]
    ) -> List[str]:
        """
        ...
        if compute_capability in self.CUDA_COMPATIBILITY_MAP:
            min_cuda, max_cuda, recommended = self.CUDA_COMPATIBILITY_MAP[
                compute_capability
            ]
    ...
    def check_cuda_compatibility(
        self, gpu: GPUInfo, target_cuda_version: str = None
    ) -> CUDACompatibility:
        """
        ...
        if target_cuda_version is None:
            target_cuda_version = (
                gpu.recommended_cuda_versions[0]
                if gpu.recommended_cuda_versions
                else "12.6"
            )
        ...
        if gpu.compute_capability < min_compute_capability:
            is_supported = False
            reasons.append(
                f"Compute capability {gpu.compute_capability} muito baixo (mínimo: {min_compute_capability})"
            )
        ...
        if gpu.compute_capability in self.CUDA_COMPATIBILITY_MAP:
            ...
            if target_cuda_version < min_cuda:
                reasons.append(
                    f"Versão CUDA {target_cuda_version} muito baixa (mínimo: {min_cuda})"
                )
            elif target_cuda_version > max_cuda:
                reasons.append(
                    f"Versão CUDA {target_cuda_version} muito alta (máximo: {max_cuda})"
                )
        ...
        if gpu.driver_version != "Unknown":
            try:
                ...
                if current_driver < required_driver:
                    is_supported = False
                    reasons.append(
                        f"Driver {gpu.driver_version} muito antigo (mínimo: {min_driver_version})"
                    )
            except ValueError:
        ...
        if gpu.memory_total < min_memory_mb:
            reasons.append(
                f"Memória GPU {gpu.memory_total}MB insuficiente (recomendado: {min_memory_mb}MB+)"
            )
    ...
    def get_best_gpu(self, gpus: List[GPUInfo]) -> Optional[GPUInfo]:
        ...
        def gpu_score(gpu: GPUInfo) -> Tuple[int, int, int, int]:
            return (
                gpu.compute_capability[0] * 10
                + gpu.compute_capability[1],  # Compute capability
        ...
        self.logger.info(
            f"Melhor GPU selecionada: {best_gpu.name} (Compute {best_gpu.compute_capability})"
        )
