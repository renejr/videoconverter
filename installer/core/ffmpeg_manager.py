...
class FFmpegManager:
    ...
    def __init__(self, cache_dir: str = None, install_dir: str = None):
        ...
        self.logger.info(f"Gerenciador FFmpeg inicializado para {self.os_type} {self.architecture}")
    ...
    def _get_default_install_dir(self) -> str:
        ...
        if self.os_type == "windows":
            return os.path.join(os.environ.get("PROGRAMFILES", "C:\\Program Files"), "FFmpeg")
    ...
    def _check_ffmpeg_in_directory(self, directory: str) -> Optional[FFmpegInstallation]:
            ...
            if result.returncode == 0 and "ii" in result.stdout:
                ...
                if os.path.exists(ffmpeg_path) and os.path.exists(ffprobe_path):
                    ...
                    return FFmpegInstallation(
                        ...
                        ffmpeg_path=ffmpeg_path,
                        ffprobe_path=ffprobe_path,
                        ffplay_path="/usr/bin/ffplay" if os.path.exists("/usr/bin/ffplay") else None,
                        is_complete=True,
                        build_info=build_info,
                        installation_date="Unknown"
                    )
    ...
    def _get_ffmpeg_build_info(self, ffmpeg_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                codec_lines = [line for line in result.stdout.split('\n') if line.strip() and not line.startswith(' ')]
    ...
    def _get_latest_windows_build(self) -> Optional[FFmpegBuild]:
        ...
        try:
            ...
            response = requests.get(
                self.FFMPEG_URLS["windows"]["releases"],
                timeout=30
            )
    ...
    def download_ffmpeg(self, build: FFmpegBuild, destination_dir: str = None) -> Optional[str]:
        """
        Baixa FFmpeg
    ...
    def install_ffmpeg(self, archive_path: str, build: FFmpegBuild) -> bool:
        ...
        install_steps = 6
        progress_bar = installation_progress_bar(install_steps, f"FFmpeg {build.version}")
        ...
        try:
            ...
            final_ffmpeg = os.path.join(self.install_dir, "ffmpeg" + (".exe" if self.os_type == "windows" else ""))
            final_ffprobe = os.path.join(self.install_dir, "ffprobe" + (".exe" if self.os_type == "windows" else ""))
            final_ffplay = os.path.join(self.install_dir, "ffplay" + (".exe" if self.os_type == "windows" else ""))
        ...
        if self.os_type == "windows":
            ...
            try:
                ...
                if self.install_dir not in current_path:
                    new_path = f"{current_path};{self.install_dir}" if current_path else self.install_dir
