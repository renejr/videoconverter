...
class CUDAInstaller:
                ...
                for line in result.stdout.split('\n'):
                    if 'cuda-toolkit-' in line and line.startswith('ii'):
                        ...
                        if len(parts) >= 3:
                            ...
                            version_match = re.search(r'cuda-toolkit-(\d+-\d+)', package_name)
                            if version_match:
    ...
    def select_best_cuda_version(self, gpu_info: GPUInfo = None) -> Optional[str]:
        ...
        if gpu_info.recommended_cuda_versions:
            for recommended_version in gpu_info.recommended_cuda_versions:
                if recommended_version in self.CUDA_VERSIONS:
                    ...
                    if os_key in cuda_version.supported_os or "linux" in cuda_version.supported_os:
                        self.logger.info(f"Versão CUDA selecionada: {recommended_version}")
        ...
        if compatible_versions:
            ...
            compatible_versions.sort(key=lambda v: tuple(map(int, v.split('.'))), reverse=True)
    ...
    def install_cuda(self, installer_path: str, version: str) -> bool:
        ...
        try:
            ...
            progress_bar.update(1, description="Verificando privilégios administrativos")
            
            if not self.os_detector.has_admin_privileges():
                self.logger.error("Privilégios administrativos necessários para instalação")
            ...
            if self.os_type == "windows":
                success = self._install_cuda_windows(installer_path, version, progress_bar)
            else:
                success = self._install_cuda_linux(installer_path, version, progress_bar)
    ...
    def _install_cuda_windows(self, installer_path: str, version: str, progress_bar: ProgressBar) -> bool:
    ...
    def _install_cuda_linux(self, installer_path: str, version: str, progress_bar: ProgressBar) -> bool:
...
"""
    ...
    def verify_installation(self, version: str) -> bool:
            ...
            if result.returncode == 0:
                ...
                if version_match:
                    ...
                    if installed_version == version[:4]:  # Comparar major.minor
                        self.logger.success(f"CUDA {installed_version} verificado via nvcc")
                        return True
                    else:
                        self.logger.warning(f"Versão diferente detectada: {installed_version}")
                        return False
        ...
        for installation in installations:
            if installation.version.startswith(version[:4]) and installation.is_complete:
                self.logger.success(f"Instalação CUDA {installation.version} verificada")
                return True
...
if __name__ == "__main__":
    ...
    if best_version:
        ...
        if version_installed:
            ...
            print(f"   Para instalar: execute o download e instalação de CUDA {best_version}")
    else:
