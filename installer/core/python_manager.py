...
class PythonManager:
    ...
    def download_python(self, progress_callback=None) -> Optional[str]:
        ...
        if not url:
            print(
                f"❌ Python 3.13 não disponível para {self.os_info.name} {self.os_info.architecture}"
            )
            return None
    ...
    def install_python(
        self, installer_path: str, custom_path: Optional[str] = None
    ) -> bool:
        """
    ...
    def _install_python_windows(
        self, installer_path: str, custom_path: Optional[str]
    ) -> bool:
    ...
    def _install_python_linux(
        ...
        try:
            ...
            # Executar configure
            result = subprocess.run(configure_args, cwd=extract_dir, timeout=300)
            ...
            # Instalar
            result = subprocess.run(
                ["sudo", "make", "altinstall"], cwd=extract_dir, timeout=300
            )
