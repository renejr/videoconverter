...
class ConversionQueueManager:
    ...
    def add_job(self, input_file: str, output_file: str, settings: Dict[str, Any], 
    ...
    def _select_optimal_gpu(self, job: ConversionJob) -> GPUType:
        ...
        if self.hardware_profile.recommended_settings.get('prefer_nvidia_for_hevc', False):
            if 'h.265' in format_name or 'hevc' in format_name:
                if self.hardware_profile.nvidia_gpus:
                    return GPUType.NVIDIA
        
        if self.hardware_profile.recommended_settings.get('prefer_intel_for_h264', False):
        ...
        with self.lock:
            ...
            if len(self.active_jobs) == 0 and self.job_queue.empty():
                ...
                if self.global_callbacks.get('queue_finished'):
                    ...
                    self.global_callbacks['queue_finished'](total_jobs, successful_jobs, failed_jobs)
