...
def test_webp_command_generation():
    ...
    try:
        ...
        try:
            ...
            print(
                f"⚠ Não foi possível gerar comando (FFmpeg pode não estar instalado): {str(e)}"
            )
    ...
    try:
        ...
        for preset in required_presets:
            if preset in WEBP_QUALITY_PRESETS:
                ...
                print(
                    f"✓ Preset '{preset}': quality={settings['quality']}, method={settings['method']}"
                )
            else:
...
def main():
    ...
    if passed == total:
        print(
            "🎉 Todos os testes passaram! WebP animado com transparência está pronto para uso."
        )
    else:
