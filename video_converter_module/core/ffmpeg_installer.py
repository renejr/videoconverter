...
class FFmpegInstaller:
    ...
    def is_ffmpeg_installed(self):
        ...
        try:
            result = subprocess.run(
                ["ffmpeg", "-version"], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=10
            )
    ...
    def extract_ffmpeg(self, archive_path):
        ...
        if self.system.startswith("win") or self.system.startswith("darwin"):
            ...
            if self.system.startswith("win"):
                ...
                extracted_folders = [
                    f
                    for f in self.ffmpeg_dir.iterdir()
    ...
    def install_ffmpeg(self, progress_callback=None):
        ...
        try:
            ...
            archive_path = self.download_ffmpeg(
                lambda p: (
                    progress_callback("Baixando FFmpeg...", p)
                    if progress_callback
                    else None
                )
            )
    ...
    def get_ffmpeg_command(self):
        ...
        project_ffmpeg = (
            Path.cwd() / "ffmpeg.exe"
            if self.system.startswith("win")
            else Path.cwd() / "ffmpeg"
        )
        if project_ffmpeg.exists():
            try:
                result = subprocess.run(
                    [str(project_ffmpeg), "-version"], capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=5
                )
                if result.returncode == 0:
