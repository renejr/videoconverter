...
# Configurações específicas para VP9/WEBM otimizadas
VP9_QUALITY_PRESETS = {
    "Baixa": {
        "crf": 30,
        "speed": 4,  # Velocidade de encoding (0=mais lento/melhor, 5=mais rápido)
        "threads": 8,  # Número de threads para encoding
        "tile_columns": 2,  # Divisão em tiles para paralelização
        "tile_rows": 1,
        "frame_parallel": 1,  # Paralelização de frames
        "auto_alt_ref": 1,  # Frames de referência alternativos
        "lag_in_frames": 16,  # Lookahead para otimização
        "row_mt": 1,  # Multi-threading por linha
    },
    "Média": {
        "crf": 25,
        "speed": 2,
    ...
    "Alta": {
        ...
        "speed": 1,
        "threads": 8,
        "tile_columns": 3,  # Mais tiles para melhor paralelização
        "tile_rows": 2,
        "frame_parallel": 1,
        "auto_alt_ref": 1,
        "lag_in_frames": 25,
...
CUDA_CONFIG = {
    ...
    "memory_optimization": {
        "enable_memory_pool": True,  # Pool de memória para reutilização
        "prealloc_size": "256M",  # Pré-alocação de memória
        "max_alloc_size": "1G",  # Máximo de alocação por operação
        "enable_unified_memory": True,  # Memória unificada CPU/GPU
    },
    
    "stream_optimization": {
        "max_concurrent_streams": 4,  # Máximo de streams simultâneas
        "enable_async_copy": True,  # Cópia assíncrona de dados
        "stream_priority": "high",  # Prioridade das streams (low, normal, high)
        "enable_overlap": True,  # Sobreposição de operações
    },
    
    "performance_tuning": {
        "enable_fast_math": True,  # Matemática rápida (menos precisão)
        "tensor_cores": "auto",  # Uso de Tensor Cores (auto, enabled, disabled)
        "memory_growth": True,  # Crescimento dinâmico de memória
        "allow_growth": True,  # Permitir crescimento de memória
        "force_gpu_compatible": True,  # Forçar compatibilidade GPU
    },
    
    "monitoring": {
        "enable_profiling": False,  # Profiling detalhado (impacta performance)
        "memory_check_interval": 5.0,  # Intervalo de verificação de memória
        "temperature_monitoring": True,  # Monitoramento de temperatura
        "utilization_monitoring": True,  # Monitoramento de utilização
        "log_gpu_stats": True,  # Log de estatísticas GPU
    },
}
