...
class TestAllFormatsConversion(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Mock para callbacks
        self.mock_callbacks = {"progress": Mock(), "complete": Mock(), "error": Mock()}
    ...
    def _test_format_command_generation(self, format_name, extension, format_type):
        ...
        with patch("os.path.exists", return_value=True), patch(
            ...
            converter.set_conversion_parameters(
                self.test_input_file, output_file, settings, self.mock_callbacks
            )
            ...
            with patch.object(
                converter, "get_video_info", return_value=mock_video_info
            ):
                ...
                try:
                    ...
                    self.assertTrue(
                        command[0] == "ffmpeg"
                        or command[0].endswith("ffmpeg.exe")
                        or command[0].endswith("ffmpeg")
                    )
                    ...
                    self.assertTrue(
                        input_file_in_command == self.test_input_file
                        or input_file_in_command.endswith(self.test_input_file)
                        or os.path.basename(input_file_in_command)
                    ...
                    self._validate_format_specific_parameters(
                        command, format_type, format_name
                    )

                    print(
                        f"✓ Comando gerado para {format_name}: {len(command)} parâmetros"
                    )
    ...
    def _validate_format_specific_parameters(self, command, format_type, format_name):
        ...
        if format_type == "video":
            ...
            self.assertTrue(
                has_video_codec,
                f"Nenhum codec de vídeo válido encontrado em: {command_str}",
            )
            ...
            self.assertTrue(
                has_quality_param,
                f"Nenhum parâmetro de qualidade encontrado em: {command_str}",
            )
            ...
            self.assertTrue(
                has_gif_codec,
                f"Nenhum codec válido para GIF encontrado em: {command_str}",
            )
            ...
            self.assertTrue(
                has_webp_codec, f"Nenhum codec WebP válido encontrado em: {command_str}"
            )
            ...
            self.assertTrue(
                has_quality_param or has_image_ext,
    ...
    def test_codec_compatibility_by_format(self):
        ...
        print(
            f"✓ Compatibilidade de codecs validada para {len(codec_format_compatibility)} codecs"
        )
    ...
    def _get_format_settings(self, format_type):
        ...
        if format_type == "gif":
            ...
            base_settings.update({"webp_quality": "Médio", "webp_fps": "15"})
        elif format_type == "frames":
            base_settings.update(
                {"frame_format": "jpg", "frame_mode": "all", "frame_interval": 1}
            )
    ...
    def test_hardware_acceleration_compatibility(self):
        ...
        print(
            f"✓ Formatos NVENC disponíveis quando CUDA está ativo: {len(nvenc_formats)}"
        )
        ...
        print(
            f"✓ Formatos CPU disponíveis quando CUDA não está ativo: {len(cpu_formats)}"
        )
    ...
    def test_frame_extraction_formats(self):
        ...
        for frame_format in FRAME_EXTRACTION_FORMATS:
            extension = self.converter_manager.get_output_extension(
                "Extração de Frames", frame_format
            )
            self.assertTrue(extension.startswith("."))

        print(
            f"✓ Formatos de extração de frames validados: {len(FRAME_EXTRACTION_FORMATS)}"
        )
