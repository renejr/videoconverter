...
class HardwareDetector:
    ...
    def _detect_ffmpeg_codecs(self) -> Dict[str, List[str]]:
        ...
        if not self.ffmpeg_command:
            logger.warning("FFmpeg não encontrado - tentando instalar automaticamente...")
            try:
        ...
        try:
            # Detecta encoders
            result = subprocess.run([self.ffmpeg_command, '-encoders'], capture_output=True, text=True, timeout=10)
            ...
            # Detecta decoders
            result = subprocess.run([self.ffmpeg_command, '-decoders'], capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                ...
                codecs['hardware_decoders'] = [
                    dec for dec in decoders 
                    if any(pattern in dec.lower() for pattern in hardware_patterns)
                ]
            
            logger.info(f"FFmpeg: {len(codecs['hardware_encoders'])} encoders de hardware detectados")
            return codecs
    ...
    def test_cuda_encoding(self) -> bool:
        ...
        try:
            ...
            test_cmd = [
                self.ffmpeg_command, '-y', '-f', 'lavfi', '-i', 'testsrc=duration=1:size=320x240:rate=1',
