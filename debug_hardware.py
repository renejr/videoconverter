...
def debug_ffmpeg():
    ...
    if os.path.exists(local_ffmpeg):
        try:
            result = subprocess.run(
                [local_ffmpeg, "-encoders"], capture_output=True, text=True, timeout=10
            )
            print(f"Return code: {result.returncode}")
...
def debug_detector():
    ...
    if detector.ffmpeg_codecs:
        print(
            f"Hardware encoders: {detector.ffmpeg_codecs.get('hardware_encoders', [])}"
        )
        print(
            f"Hardware decoders: {detector.ffmpeg_codecs.get('hardware_decoders', [])}"
        )
