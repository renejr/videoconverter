...
class ButtonStateValidator:
    ...
    def setup_callbacks(self):
        ...
        def on_job_failed_wrapper(job_data):
            ...
            self.log_event("JOB_FALHADO", 
                          f"Job {job_id} falhou: {error} "
                          f"({self.jobs_failed} falhas)")
        ...
        def on_queue_finished_wrapper(total, successful, failed):
            ...
            self.log_event("FILA_FINALIZADA", 
    ...
    def check_prerequisites(self):
        ...
        if not os.path.exists(self.output_dir):
            self.log_event("ERRO", f"Não foi possível criar diretório: {self.output_dir}")
            return False
        else:
    ...
    def run_test(self):
        ...
        try:
            ...
            def start_conversion():
                ...
                # Adicionar vídeos
                for video in self.test_videos:
                    if video not in [f['path'] for f in self.main_window.selected_files]:
                        file_info = {
                            'path': video,
                    ...
                    self.jobs_started += 1
                    self.log_event("VÍDEO_ADICIONADO", f"Adicionado: {os.path.basename(video)}")
